...
export class ExportReporter {
    ...
    if (mapping) {
      ...
      if (hasSuccess) {
        ...
        if (firstSuccess) {
          ...
          mapping.relativePath = firstSuccess.filePath
            ? path.relative(this.outputDirectory, firstSuccess.filePath)
            : '';
      ...
      const totalProcessingTime = Object.values(formatResults).reduce(
        (total, result) => total + (result.processingTime || 0),
        0
      );
  ...
  public async generateReport(
    ...
    const auditEvents =
      reportOptions.includeAuditTrail && this.auditLogger
  ...
  private generateSummary(state: ProgressState): ExportSummary {
    ...
    const exportFormats = Array.from(
      new Set(mappings.filter((m) => m.exportFormat).map((m) => m.exportFormat!))
    );
  ...
  private generateFormatSummary(mappings: ExportDocumentMapping[]): FormatSummary {
    ...
    Object.keys(formatCounts).forEach((format) => {
      ...
      formatSuccessRates[format] = counts.total > 0 ? (counts.success / counts.total) * 100 : 0;
    });
  ...
  private generateStatistics(): ExportStatistics {
    ...
    folderGroups.forEach((folderMappings, folderName) => {
      ...
      folderStats[folderName] = {
        ...
        successRate:
          folderMappings.length > 0 ? (successful.length / folderMappings.length) * 100 : 0,
      };
    });
...
<body>
    ...
    <div class="format-results">
        ...
        <table>
            ...
            <tbody>
                ${Object.entries(report.summary.formatResults.formatCounts)
                  .map(
                    ([format, counts]) => `
                    <tr>
    ...
    <div class="folder-structure">
        ...
        ${report.folderStructure.folderHierarchy
          .map(
            (folder) => `
            ...
            ${report.documentMappings
              .map((mapping) => {
                ...
                if (mapping.formatResults) {
                  ...
                  totalSize = successfulFormats.reduce(
